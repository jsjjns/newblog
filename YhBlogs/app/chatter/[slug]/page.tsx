...
async function getChatterData(slug: string) {
  const chatter = getAllChatters().find((entry) => entry.meta.slug === slug);
  if (!chatter) {
    throw new Error(`Chatter not found: ${slug}`);
  }

  let content = chatter.content;
  ...
  return {
    ...
    title: chatter.meta.title || '碎片记录',
    date: chatter.meta.date,
    mood: undefined,
    tags: Array.isArray(chatter.meta.tags) ? chatter.meta.tags : [],
    cover: siteConfig.defaultPostCover
  };
}

function getRecentChatters(currentSlug: string) {
  return getAllChatters()
    .filter((entry) => entry.meta.slug !== currentSlug)
    .map((entry) => ({
      slug: entry.meta.slug,
      title: entry.meta.title || '碎片记录',
