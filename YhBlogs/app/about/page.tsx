...
import { getAboutDocument } from '../../lib/content/about';
...
function getDirActivities(dirName: string, typeLabel: '文章' | '杂谈' | '说说', linkPrefix: string) {
  if (dirName === 'posts') {
    return getAllPosts().map((post) => ({
      id: `${dirName}-${post.meta.slug}`,
      type: typeLabel,
      title: post.meta.title || post.meta.slug,
      date: post.meta.date ? new Date(post.meta.date).toISOString() : '1970-01-01T00:00:00Z',
      url: `/${linkPrefix}/${post.meta.slug}`,
    }));
  }

  if (dirName === 'chatters') {
    return getAllChatters().map((entry) => ({
      id: `${dirName}-${entry.meta.slug}`,
      type: typeLabel,
      title: entry.meta.title || entry.meta.slug,
      date: entry.meta.date ? new Date(entry.meta.date).toISOString() : '1970-01-01T00:00:00Z',
      url: `/${linkPrefix}/${entry.meta.slug}`,
    }));
  }
...
export default async function AboutPage() {
  ...
  try {
    ...
    if (aboutDocument?.cover) coverImage = aboutDocument.cover;
